"""
功能描述：bert encoder类，用bert编码文本，融合了whitening方法，可以有效的提高bert的检索能力支持使用bert_service，利用远端gpu能力encode bert
更新记录：
    - 20220402 支持链接bert service，并且支持配置超时秒数，这个功能主要用在在线服务的时候，若bert service服务挂掉还可以使用cpu encode
    - 20220612 支持自动whitening
    - 20221212 支持指定embedding输出位置和层
"""
...
from backend.utils.encoder_utils import timeout, EncodeDataGenerator, merge
from backend.third_party_components.vecs_whitening import VecsWhitening
...
class BertEncoder(object):
    ...
    def __init__(
            ...
            encode_one_sample_max_second: float = None,
            **kwargs
    ):
        self.kwargs = kwargs
        ...
        self.encode_one_sample_max_second = encode_one_sample_max_second or float("inf")
    ...
    def __test_service_encode(self, text: str):
        ...
        @timeout(self.encode_one_sample_max_second)
        def __test_service_encode_wrapper(string: str):
            # 拿一条数据测试，设置超时时间20ms，如果超时则不使用bert_service
            self.bert_client.encode([string])
        return __test_service_encode_wrapper(text)
