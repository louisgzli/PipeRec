...
def str2list(input_str: str, sep: str = ",", trans_type: str = "str") -> List[str]:
    ...
    return [type_map[trans_type](i.strip()) for i in input_str.split(sep) if i.strip()]


def str2dict(input_str: str, trans_type: type = str) -> Dict[str, str]:
    """
    ...
    for params in input_str.strip().split(";"):
        ...
        res[key.strip()] = trans_type(value.strip())
    return res
